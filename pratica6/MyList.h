...
public:
	...
	void reverse();	
	void reverse(Node<T> *atual, Node<T> *before);																		//isso sera melhorado no futuro (quando estudarmos iteradores)
	iterator erase(iterator elem); //remove o elemento apontado por Elem//retorna o (apontador) para o elemento apos o removido.
...
template<class T>
void MyList<T>::destroy(MyList<T>::iterator firstelem){
	if(firstelem == NULL){
		return;
	}
	destroy(firstelem->next);
	delete firstelem;
}
...
void MyList<T>::destroy(){ 	
	Node<T> *newNode = NULL;
	while(dataFirst){
		newNode = dataFirst; //atribui o dataFirst para o nodo auxiliar
		dataFirst = newNode->next; //datafirst recebe o prox do nodo auxiliar, que no caso eh o prox elemento da lista
		delete newNode;
	}
...
template<class T>
void MyList<T>::reverse(Node<T> *atual, Node<T> *before){
		if(atual->next != NULL){
			reverse(atual->next, atual);
		}
		atual->next = before;
}
template<class T>
...
void MyList<T>::push_front(const T&elem) {
	if(dataFirst == NULL){ //caso especial de lista vazia
		dataFirst = dataLast = new Node<T>(elem);
	}
	else{
		Node<T> *newNode = new Node<T>(elem);
		newNode->next = dataFirst; //o novo nodo recebe o primeiro elemento como proximo
		dataFirst = newNode; //o primeiro elemento recebe o novo nodo
	}
	dataSize++; //incrementa o datasize pois inserimos um elemento na lista 
}
