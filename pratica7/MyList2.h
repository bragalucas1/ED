...
void MyList2<T>::push_front(const T&elem) {
	if(dataFirst == NULL){ //caso especial de lista vazia
		push_back(elem); //fazemos o push_back do elem, a fim de reparoveitar função e não duplicar código
	}
	else{
		...
		dataFirst = newNode;
		dataSize++; //incrementa-se dentro do else a fim de não duplicarmos o ds
	}
}
...
template <class T>
void MyList2<T>::reverse(Node<T> *n){
	if(n->next!=NULL){
		reverse(n->next);
	}
	swap(n->prev,n->next);
...
void MyList2<T>::insert(const T&elem, iterator where) {
	if(where == end()) { //caso especial: insercao no final da lista... 
		push_back(elem);
	} else if (where==begin()) { //caso especial: insercao no inicio da lista
		Node<T> *newNode = new Node<T>(elem);
		newNode->next = dataFirst;
		dataFirst->prev = newNode;
		dataFirst = newNode;
